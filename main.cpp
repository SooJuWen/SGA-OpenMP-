...
#pragma omp parallel
		{
			...
			//Procedural loop
			for (int count = id; count < 5; count = count + nthrds) {

				//Display the round number
#pragma omp critical
				cout << endl << "Round: " << count + 1 << " (Thread - " << id << ")" << endl;
...
#pragma omp critical
				...
				//wait all the threads finish execute their loop here, only go for another pop size procedure if needed.
#pragma omp barrier
#pragma omp single
				cout << "------------------End of one procedural loop------------------" << endl << endl;
			}
	...
	for (int i = 0; i < 5; i++) {
		if (passInt[i] == 1)
			cout << "Thread " << i << " (Round " << i + 1 << "): Passed" << endl;
		else
			cout << "Thread " << i << " (Round " << i + 1 << "): Failed" << endl;
	}
